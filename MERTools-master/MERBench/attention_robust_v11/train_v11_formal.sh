...
BASE_SEED="${BASE_SEED:-4407}"
REPEATS="${REPEATS:-6}"
PROFILE_SET="${PROFILE_SET:-balanced}"  # balanced | multi | full
FAILED_RUNS=0
...
echo "==== V11 formal stage start (repeats=${REPEATS}, base_seed=${BASE_SEED}, profile_set=${PROFILE_SET}) ===="
...
for ((i=0; i<REPEATS; i++)); do
    ...
    case "${PROFILE_SET}" in
        balanced)
            run_case "f01_balanced_smoothl1" "${seed}" \
                --dropout=0.35 \
                --kl_weight=0.0005 \
                --recon_weight=0.012 \
        ...
        full)
            run_case "f01_balanced_smoothl1" "${seed}" \
                ...
                --lr=4e-4 --l2=5e-5 --early_stopping_patience=50 --lr_patience=12 \
            ...
            ;;
        *)
            echo "Unknown PROFILE_SET=${PROFILE_SET}. Use balanced | multi | full."
            exit 1
            ;;
    esac
done
...
run_cmd "${PYTHON_BIN}" attention_robust_v11/summarize_v11.py \
    ...
    --tag_prefix "f0" \
    --out_csv "${OUTPUT_DIR}/summary/formal_summary.csv"

if [[ "${FAILED_RUNS}" -gt 0 ]]; then
    echo "[WARN] formal stage finished with failed_runs=${FAILED_RUNS}"
fi
echo "==== V11 formal stage done ===="
