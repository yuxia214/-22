...
BASE_SEED="${BASE_SEED:-6407}"
REPEATS="${REPEATS:-2}"
BATCH_SIZE="${BATCH_SIZE:-16}"  # stronger parallel safety for same-GPU multi jobs
FAILED_RUNS=0
...
echo "==== V11 group E start (repeats=${REPEATS}, base_seed=${BASE_SEED}) ===="

for ((i=0; i<REPEATS; i++)); do
    seed=$((BASE_SEED + i))

    # e01: noise-robust + lower-batch parallel profile
    run_case "e01_parallel_noise_guard" "${seed}" \
        --dropout=0.35 \
        --kl_weight=0.0008 \
        --recon_weight=0.015 \
        --cross_kl_weight=0.0013 \
        --fusion_temperature=0.38 \
        --modality_dropout=0.28 \
        ...
        --lr_patience=12 \
        --emo_loss_weight=1.0 \
        --val_loss_weight=1.25 \
        --reg_loss_type=smoothl1 \
        --huber_beta=0.7

    # e02: stable MSE profile for higher Average score
    run_case "e02_parallel_stable" "${seed}" \
        --dropout=0.30 \
        --kl_weight=0.0005 \
        --recon_weight=0.010 \
        --cross_kl_weight=0.0010 \
        --fusion_temperature=0.28 \
...
run_cmd "${PYTHON_BIN}" attention_robust_v11/summarize_v11.py \
    ...
    --tag_prefix "e0" \
    --out_csv "${OUTPUT_DIR}/summary/group_e_summary.csv"

if [[ "${FAILED_RUNS}" -gt 0 ]]; then
    echo "[WARN] group E finished with failed_runs=${FAILED_RUNS}"
fi
echo "==== V11 group E done ===="
