...
REPEATS="${REPEATS:-2}"
SHORT_EPOCHS="${SHORT_EPOCHS:-30}"
SHORT_EARLY_STOP="${SHORT_EARLY_STOP:-10}"
SHORT_LR_PATIENCE="${SHORT_LR_PATIENCE:-4}"
FAILED_RUNS=0
...
run_case() {
    ...
    echo ""
    echo "==================================================================="
    echo "run_tag=${run_tag} seed=${seed} gpu=${GPU_ID}"
...
echo "==== V11 short stage start (repeats=${REPEATS}, base_seed=${BASE_SEED}) ===="

for ((i=0; i<REPEATS; i++)); do
    seed=$((BASE_SEED + i))

    # c01: v2 best historical profile (strong MER-MULTI single-run baseline)
    run_case "c01_v2best_emoval" "${seed}" \
        --dropout=0.35 \
        --kl_weight=0.0005 \
        --recon_weight=0.01 \
        ...
        --l2=5e-5 \
        --epochs="${SHORT_EPOCHS}" \
        --early_stopping_patience="${SHORT_EARLY_STOP}" \
        --lr_patience="${SHORT_LR_PATIENCE}" \
        --emo_loss_weight=1.0 \
        --val_loss_weight=1.0 \
        --reg_loss_type=mse

    # c02: MER-MULTI focused profile (classification-priority selection)
    run_case "c02_multi_focus" "${seed}" \
        --dropout=0.35 \
        --kl_weight=0.0005 \
        --recon_weight=0.01 \
        --cross_kl_weight=0.001 \
    ...
    # c03: balanced profile (stabilize MSE and keep WAF)
    run_case "c03_balanced_smoothl1" "${seed}" \
        --dropout=0.35 \
        --kl_weight=0.0005 \
        --recon_weight=0.012 \
        --cross_kl_weight=0.001 \
    ...
    # c04: noise-robust profile (improve MER-NOISE stability)
    run_case "c04_noise_guard" "${seed}" \
        --dropout=0.35 \
        --kl_weight=0.0007 \
        --recon_weight=0.015 \
        --cross_kl_weight=0.0012 \
        --fusion_temperature=0.35 \
    ...
    # c05: upper-bound clean profile (often high MER-MULTI, may hurt NOISE)
    run_case "c05_clean_upper" "${seed}" \
        --dropout=0.35 \
        --kl_weight=0.0003 \
        --recon_weight=0.008 \
...
run_cmd "${PYTHON_BIN}" attention_robust_v11/summarize_v11.py \
    ...
    --tag_prefix "c0" \
    --out_csv "${OUTPUT_DIR}/summary/short_summary.csv"

if [[ "${FAILED_RUNS}" -gt 0 ]]; then
    echo "[WARN] short stage finished with failed_runs=${FAILED_RUNS}"
fi
echo "==== V11 short stage done ===="
