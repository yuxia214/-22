...
def train_or_eval_model(args, model, reg_loss, cls_loss, dataloader, dataloader_class, epoch, optimizer=None, train=False, task_weighter=None):
    ...
    for iter, data in enumerate(dataloader):
        ...
        vidnames += bnames
        for key in batch: batch[key] = batch[key].cuda(non_blocking=True)
...
if __name__ == '__main__':
    ...
    for ii in range(len(train_loaders)):
        ...
        for epoch in range(args.epochs):
            ...
            train_results = train_or_eval_model(args, model, reg_loss, cls_loss, train_loader, dataloader_class, epoch=epoch, optimizer=optimizer, train=True, task_weighter=task_weighter)
            eval_results  = train_or_eval_model(args, model, reg_loss, cls_loss, eval_loader, dataloader_class, epoch=epoch, optimizer=None,      train=False, task_weighter=task_weighter)
            ...
            if args.test_each_epoch:
                for jj, test_loader in enumerate(test_loaders):
                    test_results = train_or_eval_model(args, model, reg_loss, cls_loss, test_loader, dataloader_class, epoch=epoch, optimizer=None, train=False, task_weighter=task_weighter)
        ...
        eval_model = swa_model if (args.use_swa and swa_updates > 0) else model
        if args.use_swa and swa_updates > 0:
            best_eval_results = train_or_eval_model(args, eval_model, reg_loss, cls_loss, eval_loader, dataloader_class, epoch=best_epoch, optimizer=None, train=False, task_weighter=task_weighter)
        elif best_eval_results is None:
            best_eval_results = train_or_eval_model(args, eval_model, reg_loss, cls_loss, eval_loader, dataloader_class, epoch=best_epoch, optimizer=None, train=False, task_weighter=task_weighter)
        best_store = {}
        func_update_storage(inputs=best_eval_results, prefix='eval', outputs=best_store)

        for jj, test_loader in enumerate(test_loaders):
            test_results = train_or_eval_model(args, eval_model, reg_loss, cls_loss, test_loader, dataloader_class, epoch=best_epoch, optimizer=None, train=False, task_weighter=task_weighter)
