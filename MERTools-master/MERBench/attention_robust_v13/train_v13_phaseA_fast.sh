...
run_case() {
    ...
    if run_cmd "${PYTHON_BIN}" attention_robust_v13/launch_main_robust_seeded.py \
        --seed="${SEED}" \
...
echo "==== V13 Phase-A fast start (seed=${SEED}, epochs=${EPOCHS}) ===="

# b1: loss升级 + stage2
run_case "b1_v13_lossup_stage2" \
...
# b3: loss升级 + prior + stage2
run_case "b3_v13_both_stage2" \
    --use_uncertainty_weighted_mt \
    --mt_init_log_var_cls=0.00 \
...
# b4: loss升级 + prior + stage2 + SWA + calibration
run_case "b4_v13_both_stage2_swa_calib" \
    --use_uncertainty_weighted_mt \
...
run_cmd "${PYTHON_BIN}" attention_robust_v13/summarize_v13.py \
    --root "${OUTPUT_DIR}/results" \
    --tag_prefix "b" \
    --out_csv "${OUTPUT_DIR}/summary/phaseA_fast_summary.csv"

if [[ "${FAILED_RUNS}" -gt 0 ]]; then
    echo "[WARN] V13 phase-A fast finished with failed_runs=${FAILED_RUNS}"
fi
echo "==== V13 Phase-A fast done ===="
