...
LOG_DIR="${MERBENCH_ROOT}/attention_robust_v16/outputs/logs"
mkdir -p "${LOG_DIR}"
...
for i in "${!CASES[@]}"; do
    ...
    cmd="cd ${MERBENCH_ROOT} && GPU_ID=${GPU_ID} SEED=${SEED} THREADS_PER_RUN=${THREADS_PER_RUN} NUM_WORKERS=${NUM_WORKERS} BATCH_SIZE=${BATCH_SIZE} EPOCHS=${EPOCHS} CPU_CORES=${cpu_cores} CASE_ID=${case_id} RUN_TAG=${run_tag} bash attention_robust_v16/train_v16_single_case.sh 2>&1 | tee ${log_file}"
