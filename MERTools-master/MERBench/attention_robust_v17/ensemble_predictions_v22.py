...
4) Optional post-ensemble valence calibration (linear / isotonic).
"""
import argparse
import glob
import json
...
def fit_linear_calibration(preds: np.ndarray, labels: np.ndarray) -> Tuple[float, float]:
...
def apply_linear_calibration(preds: np.ndarray, coef: float, bias: float) -> np.ndarray:
    x = np.asarray(preds).astype(np.float64)
    return coef * x + bias


def fit_post_reg_calibrator(
    mode: str,
    emo_probs: np.ndarray,
    val_preds: np.ndarray,
    val_labels: np.ndarray,
    ...
    if preds.size == 0 or labels.size == 0 or preds.size != labels.size:
        return None, {"mode": mode, "warning": "empty_or_mismatched_data"}

    if mode == "linear":
        coef, bias = fit_linear_calibration(preds, labels)
        return {"mode": mode, "coef": coef, "bias": bias}, {
            "mode": mode,
            "coef": coef,
            "bias": bias,
        }
    ...
    if mode == "isotonic":
        ...
        return {"mode": mode, "model": iso}, {
            "mode": mode,
            "n_knots": int(len(getattr(iso, "X_thresholds_", []))),
        }

    pred_cls = np.argmax(probs, axis=1).astype(np.int64)
    unique_cls = sorted(int(x) for x in np.unique(pred_cls))

    if mode == "pred_cls_linear":
        g_coef, g_bias = fit_linear_calibration(preds, labels)
        groups = {}
        kept = []
        for c in unique_cls:
            idx = pred_cls == c
            if int(np.sum(idx)) < max(1, int(min_samples)):
                continue
            coef, bias = fit_linear_calibration(preds[idx], labels[idx])
            groups[int(c)] = (float(coef), float(bias))
            kept.append(int(c))
        return {
            "mode": mode,
            "global_coef": g_coef,
    ...
    if mode == "pred_cls_isotonic":
        global_iso = IsotonicRegression(out_of_bounds="clip")
        global_iso.fit(preds, labels)
        groups = {}
        kept = []
        for c in unique_cls:
            idx = pred_cls == c
        ...
        return {
            "mode": mode,
            "global_model": global_iso,
            "groups": groups,
        }, {
            "mode": mode,
            "global_knots": int(len(getattr(global_iso, "X_thresholds_", []))),
            "group_count": len(kept),
            "groups": kept,
        }

    raise ValueError(f"unsupported post_reg_calibration mode: {mode}")


def apply_post_reg_calibration(
    calibration: Optional[Dict],
    emo_probs: np.ndarray,
    val_preds: np.ndarray,
    clip_abs: float,
) -> np.ndarray:
    preds = np.asarray(val_preds).astype(np.float64)
    if calibration is None:
        ...
        if mode == "linear":
            out = apply_linear_calibration(flat, calibration["coef"], calibration["bias"])
        elif mode == "isotonic":
            out = calibration["model"].predict(flat)
        elif mode == "pred_cls_linear":
            probs = np.asarray(emo_probs)
            pred_cls = np.argmax(probs, axis=1).astype(np.int64)
            out = apply_linear_calibration(flat, calibration["global_coef"], calibration["global_bias"])
            for c, (coef, bias) in calibration["groups"].items():
                idx = pred_cls == int(c)
                if np.any(idx):
                    out[idx] = apply_linear_calibration(flat[idx], coef, bias)
        elif mode == "pred_cls_isotonic":
            probs = np.asarray(emo_probs)
            pred_cls = np.argmax(probs, axis=1).astype(np.int64)
            out = calibration["global_model"].predict(flat)
            for c, iso in calibration["groups"].items():
                idx = pred_cls == int(c)
                if np.any(idx):
                    out[idx] = iso.predict(flat[idx])
        else:
            raise ValueError(f"unknown calibration mode: {mode}")

    if clip_abs > 0:
        out = np.clip(out, -clip_abs, clip_abs)
    return np.asarray(out, dtype=np.float32).reshape(preds.shape)
...
def tune_score_with_objective(
    models: List[ModelPack],
    tune_set: str,
    w_cls: np.ndarray,
    w_reg: np.ndarray,
    search_objective: str,
    search_calibration_mode: str,
    calibration_min_samples: int,
    calibration_clip: float,
) -> Tuple[float, Dict[str, float]]:
    raw_metrics, emo_probs, emo_labels, val_preds, val_labels, _, _, _ = combine_on_set(
        models,
        tune_set,
        w_cls,
        w_reg,
    )
    if search_objective == "raw":
        return float(raw_metrics["combined"]), raw_metrics

    try:
        calibration, _ = fit_post_reg_calibrator(
            mode=search_calibration_mode,
            emo_probs=emo_probs,
            val_preds=val_preds,
            val_labels=val_labels,
            min_samples=max(1, int(calibration_min_samples)),
        )
        val_preds_cal = apply_post_reg_calibration(
            calibration=calibration,
            emo_probs=emo_probs,
            val_preds=val_preds,
            clip_abs=float(calibration_clip),
        )
        metrics = evaluate_pack(emo_probs, emo_labels, val_preds_cal, val_labels)
        return float(metrics["combined"]), metrics
    except Exception:
        # Fail-safe: if calibration fails for a candidate, fall back to raw metric.
        return float(raw_metrics["combined"]), raw_metrics
...
def forward_sparse_dual_search(
    ...
    for i in range(n):
        ...
        wc[i] = 1.0
        wr[i] = 1.0
        score, m = tune_score_with_objective(
            models=models,
            tune_set=tune_set,
            w_cls=wc,
            w_reg=wr,
            search_objective=search_objective,
            search_calibration_mode=search_calibration_mode,
    ...
    score_best, metrics_best = tune_score_with_objective(
        models=models,
        tune_set=tune_set,
        w_cls=wc_best,
        w_reg=wr_best,
        ...
        for cand in range(n):
            ...
            for a_cls in alpha_grid:
                for a_reg in alpha_grid:
                    ...
                    score_trial, metrics_trial = tune_score_with_objective(
                        models=models,
                        tune_set=tune_set,
                        w_cls=wc_trial,
                        w_reg=wr_trial,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="v22 ensemble: dual weights + sparse search + post-reg calibration")
    ...
    parser.add_argument(
        ...
        help="objective used in weight search",
    )
    parser.add_argument(
        "--post_reg_calibration",
        type=str,
        default="none",
        choices=["none", "linear", "isotonic", "pred_cls_linear", "pred_cls_isotonic"],
        help="post-ensemble calibration mode for valence prediction",
    )
    parser.add_argument("--calibration_min_samples", type=int, default=12, help="min samples per class for group calibration")
    parser.add_argument("--calibration_clip", type=float, default=-1.0, help="if >0, clip calibrated valence into [-clip, clip]")
    ...
    parser.add_argument("--save_prefix", default="v22", help="output file prefix")
    ...
    print(f"search_objective={args.search_objective}")
    print(f"post_reg_calibration={args.post_reg_calibration} min_samples={args.calibration_min_samples} clip={args.calibration_clip}")
    ...
    search_calibration_mode = args.post_reg_calibration if args.search_objective == "calibrated" else "none"
    ...
    w_cls_sparse, w_reg_sparse, selected, sparse_metrics = forward_sparse_dual_search(
        ...
        calibration_clip=float(args.calibration_clip),
    )
    ...
    tune_metrics_raw, tune_emo_probs, tune_emo_labels, tune_val_preds_raw, tune_val_labels, _, _, _ = combine_on_set(
        models,
        args.tune_set,
        w_cls_final,
        w_reg_final,
    )
    tune_search_score, tune_search_metrics = tune_score_with_objective(
        models=models,
        tune_set=args.tune_set,
        w_cls=w_cls_final,
        w_reg=w_reg_final,
    ...
    calibration = None
    calibration_meta = {"mode": "none"}
    if args.post_reg_calibration != "none":
        try:
            calibration, calibration_meta = fit_post_reg_calibrator(
                mode=args.post_reg_calibration,
                emo_probs=tune_emo_probs,
                val_preds=tune_val_preds_raw,
                val_labels=tune_val_labels,
                min_samples=max(1, int(args.calibration_min_samples)),
            )
        except Exception as e:
            print(f"[WARN] failed to fit post-reg calibration: {e}")
            calibration = None
            calibration_meta = {"mode": "none", "warning": str(e)}

    tune_val_preds_final = apply_post_reg_calibration(
        calibration=calibration,
        emo_probs=tune_emo_probs,
        val_preds=tune_val_preds_raw,
        clip_abs=float(args.calibration_clip),
    )
    tune_metrics_final = evaluate_pack(
        tune_emo_probs,
        tune_emo_labels,
        tune_val_preds_final,
        tune_val_labels,
    )
    print("\nTune-set metrics:")
    print(f"  raw={format_metrics(tune_metrics_raw)}")
    print(f"  search_objective={format_metrics(tune_search_metrics)} score={tune_search_score:.6f}")
    if calibration is not None:
        print(f"  calibrated={format_metrics(tune_metrics_final)}")

    report = {
        "tune_set": args.tune_set,
        "eval_sets": args.eval_sets,
        "tune_metrics_raw": tune_metrics_raw,
        "tune_metrics_search_objective": tune_search_metrics,
        "tune_metrics": tune_metrics_final,
        "search_tune_metrics": refine_metrics,
        "search_objective": args.search_objective,
        "search_calibration_mode": search_calibration_mode,
        ...
        "post_reg_calibration": args.post_reg_calibration,
        "calibration_min_samples": int(args.calibration_min_samples),
        "calibration_clip": float(args.calibration_clip),
    ...
    for set_name in args.eval_sets:
        try:
            metrics_raw, emo_probs, emo_labels, val_preds_raw, val_labels, idxs, wc_sub, wr_sub = combine_on_set(
                models,
                set_name,
                w_cls_final,
                w_reg_final,
        ...
        val_preds_final = apply_post_reg_calibration(
            calibration=calibration,
            emo_probs=emo_probs,
            val_preds=val_preds_raw,
            clip_abs=float(args.calibration_clip),
        )
        metrics = evaluate_pack(emo_probs, emo_labels, val_preds_final, val_labels)

        print(f"\n[{set_name}] raw={format_metrics(metrics_raw)}")
        if calibration is not None:
            print(f"[{set_name}] calibrated={format_metrics(metrics)}")

        report[f"metrics_raw_{set_name}"] = metrics_raw
