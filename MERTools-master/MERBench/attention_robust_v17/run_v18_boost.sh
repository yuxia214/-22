...
SEARCH_TRIALS="${SEARCH_TRIALS:-4000}"
PARALLEL_JOBS="${PARALLEL_JOBS:-4}"
THREADS_PER_RUN="${THREADS_PER_RUN:-3}"
...
echo "================================================================"
echo " V18 Boost Pipeline (on top of main-robust-v17)"
echo " Cases: ${CASES[*]}"
echo " Seeds: ${SEEDS[*]}"
...
mkdir -p "${OUTPUT_DIR}/logs" "${OUTPUT_DIR}/results" "${OUTPUT_DIR}/summary" "${OUTPUT_DIR}/ensemble_v18"
...
run_single() {
    ...
    local log_file="${OUTPUT_DIR}/logs/v18_${run_tag}_$(date +%Y%m%d_%H%M%S).log"
    ...
    if [[ -n "${EPOCHS}" ]]; then
        ...
        CPU_CORES="${cpu_cores}" \
            bash "${V17_DIR}/train_v18_single_case.sh" > "${log_file}" 2>&1
    else
        CASE_ID="${cfg}" \
        SEED="${seed}" \
        RUN_TAG="${run_tag}" \
...
# ============================================================
# Phase 3: auto-search ensemble
# ============================================================
echo ""
echo "======== Phase 3: Auto Ensemble Search ========"

ENSEMBLE_OUTPUT="${OUTPUT_DIR}/ensemble_v18"
...
echo ""
echo "================================================================"
echo " V18 Boost Pipeline Complete"
