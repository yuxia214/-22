...
SEARCH_TRIALS="${SEARCH_TRIALS:-15000}"
ALPHA_GRID="${ALPHA_GRID:-0.03,0.05,0.08,0.10,0.15,0.20,0.30,0.40,0.50,0.65}"
CORR_PENALTY="${CORR_PENALTY:-0.01}"
CORR_THRESHOLD="${CORR_THRESHOLD:-0.97}"
...
mkdir -p "${OUTPUT_DIR}/logs" "${OUTPUT_DIR}/results" "${OUTPUT_DIR}/summary" "${OUTPUT_DIR}/ensemble_v20"
...
run_single() {
    ...
    local log_file="${OUTPUT_DIR}/logs/v20_${run_tag}_$(date +%Y%m%d_%H%M%S).log"
    ...
    if [[ -n "${EPOCHS}" ]]; then
        ...
        CPU_CORES="${cpu_cores}" \
            bash "${V17_DIR}/train_v20_single_case.sh" > "${log_file}" 2>&1
    else
        CASE_ID="${cfg}" \
        SEED="${seed}" \
        RUN_TAG="${run_tag}" \
...
echo ""
echo "======== Phase 3: v20 Ensemble (dual-weight search) ========"

ENSEMBLE_OUTPUT="${OUTPUT_DIR}/ensemble_v20"
...
"${PYTHON_BIN}" "${V17_DIR}/ensemble_predictions_v19.py" \
    ...
    --save_prefix "v20_${TUNE_SET}"

echo ""
echo "================================================================"
echo " V20 Boost Pipeline Complete"
