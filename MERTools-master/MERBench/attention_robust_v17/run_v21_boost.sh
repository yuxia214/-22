...
ENSEMBLE_SCRIPT="${ENSEMBLE_SCRIPT:-${V17_DIR}/ensemble_predictions_v19.py}"
...
mkdir -p "${OUTPUT_DIR}/logs" "${OUTPUT_DIR}/results" "${OUTPUT_DIR}/summary" "${OUTPUT_DIR}/ensemble_v21"
...
run_single() {
    ...
    local log_file="${OUTPUT_DIR}/logs/v21_${run_tag}_$(date +%Y%m%d_%H%M%S).log"
    ...
    if [[ -n "${EPOCHS}" ]]; then
        ...
        CPU_CORES="${cpu_cores}" \
            bash "${V17_DIR}/train_v21_single_case.sh" > "${log_file}" 2>&1
    else
        CASE_ID="${cfg}" \
        SEED="${seed}" \
        RUN_TAG="${run_tag}" \
...
echo ""
echo "======== Phase 3: v21 Ensemble ========"

ENSEMBLE_OUTPUT="${OUTPUT_DIR}/ensemble_v21"
...
"${PYTHON_BIN}" "${ENSEMBLE_SCRIPT}" \
    ...
    --save_prefix "v21_${TUNE_SET}"

echo ""
echo "================================================================"
echo " V21 Boost Pipeline Complete"
