...
SEARCH_TRIALS="${SEARCH_TRIALS:-22000}"
...
SEARCH_OBJECTIVE="${SEARCH_OBJECTIVE:-calibrated}"
POST_REG_CALIB="${POST_REG_CALIB:-isotonic}"
...
mkdir -p "${OUTPUT_DIR}/logs" "${OUTPUT_DIR}/results" "${OUTPUT_DIR}/summary" "${OUTPUT_DIR}/ensemble_v22"
...
run_single() {
    ...
    local log_file="${OUTPUT_DIR}/logs/v22_${run_tag}_$(date +%Y%m%d_%H%M%S).log"
    ...
    if [[ -n "${EPOCHS}" ]]; then
        ...
        CPU_CORES="${cpu_cores}" \
            bash "${V17_DIR}/train_v22_single_case.sh" > "${log_file}" 2>&1
    else
        CASE_ID="${cfg}" \
        SEED="${seed}" \
        RUN_TAG="${run_tag}" \
...
echo ""
echo "======== Phase 3: v22 Ensemble ========"

ENSEMBLE_OUTPUT="${OUTPUT_DIR}/ensemble_v22"
