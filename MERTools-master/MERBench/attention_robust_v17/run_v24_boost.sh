...
# v24 default: use CV for weight search, optimize RAW Combined.
...
mkdir -p "${OUTPUT_DIR}/logs" "${OUTPUT_DIR}/results" "${OUTPUT_DIR}/summary" "${OUTPUT_DIR}/ensemble_v24"
...
run_single() {
    ...
    local log_file="${OUTPUT_DIR}/logs/v24_${run_tag}_$(date +%Y%m%d_%H%M%S).log"
    ...
    if [[ -n "${EPOCHS}" ]]; then
        ...
        CPU_CORES="${cpu_cores}" \
            bash "${V17_DIR}/train_v24_single_case.sh" > "${log_file}" 2>&1
    else
        CASE_ID="${cfg}" \
        SEED="${seed}" \
        RUN_TAG="${run_tag}" \
...
echo ""
echo "======== Phase 3: v24 Ensemble ========"

ENSEMBLE_OUTPUT="${OUTPUT_DIR}/ensemble_v24"
...
run_ensemble_once "${TUNE_SET}" "${SEARCH_OBJECTIVE}" "${POST_REG_CALIB}" "v24_${TUNE_SET}_${SEARCH_OBJECTIVE}" "${SEARCH_TRIALS}" "42" "${CORR_PENALTY}" "${MAX_MODELS}"

if [[ "${ENSEMBLE_DUAL_RAW}" == "1" && "${SEARCH_OBJECTIVE}" == "raw" && "${TUNE_SET}" != "test1" ]]; then
    echo ""
    echo "======== Phase 3b: v24 Ensemble (test1 raw safety run) ========"
    ...
    for safe_seed in "${RAW_SAFETY_SEEDS[@]}"; do
        run_ensemble_once \
            ...
            "raw" \
            "none" \
            "v24_test1_raw_safety_s${safe_seed}" \
            "${RAW_SAFETY_TRIALS}" \
            "${safe_seed}" \
...
echo ""
echo "================================================================"
echo " V24 Boost Pipeline Complete"
