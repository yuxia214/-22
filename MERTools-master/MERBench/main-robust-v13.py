...
if __name__ == '__main__':
    ...
    for ii in range(len(train_loaders)):
        ...
        if args.use_swa and swa_updates > 0:
            best_eval_results = train_or_eval_model(args, eval_model, best_epoch_reg_loss, cls_loss, eval_loader, dataloader_class, epoch=best_epoch, optimizer=None, train=False, task_weighter=task_weighter)
        elif best_eval_results is None:
            best_eval_results = train_or_eval_model(args, eval_model, best_epoch_reg_loss, cls_loss, eval_loader, dataloader_class, epoch=best_epoch, optimizer=None, train=False, task_weighter=task_weighter)

        calibration_coef = None
        calibration_bias = None
        if args.use_valence_calibration and args.output_dim2 != 0 and ('valpreds' in best_eval_results) and ('vallabels' in best_eval_results):
        ...
        for jj, test_loader in enumerate(test_loaders):
            test_results = train_or_eval_model(args, eval_model, best_epoch_reg_loss, cls_loss, test_loader, dataloader_class, epoch=best_epoch, optimizer=None, train=False, task_weighter=task_weighter)
            if calibration_coef is not None:
