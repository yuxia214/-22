...
if __name__ == '__main__':
    ...
    parser.add_argument('--cls_loss_type', type=str, default='ce',
        choices=['ce', 'label_smoothing', 'focal', 'focal_label_smoothing'],
    ...
    for ii in range(len(train_loaders)):
        ...
        if args.cls_loss_type == 'label_smoothing' and LabelSmoothingCrossEntropy is not None:
            cls_loss = LabelSmoothingCrossEntropy(smoothing=args.label_smoothing).cuda()
            print(f'fold:{ii+1}; using LabelSmoothingCrossEntropy(smoothing={args.label_smoothing})')
        elif args.cls_loss_type == 'focal' and FocalLoss is not None:
            cls_loss = FocalLoss(gamma=args.focal_gamma).cuda()
            print(f'fold:{ii+1}; using FocalLoss(gamma={args.focal_gamma})')
        elif args.cls_loss_type == 'focal_label_smoothing' and FocalLabelSmoothingLoss is not None:
            cls_loss = FocalLabelSmoothingLoss(gamma=args.focal_gamma, smoothing=args.label_smoothing).cuda()
            print(f'fold:{ii+1}; using FocalLabelSmoothingLoss(gamma={args.focal_gamma}, smoothing={args.label_smoothing})')
        else:
    ...
    for jj in range(len(test_loaders)):
        ...
        print (f'save results in {save_path}')
        # Save predictions alongside args for ensemble
