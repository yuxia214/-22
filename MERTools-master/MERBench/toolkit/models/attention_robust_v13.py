'''
AttentionRobustV13 - 基于P-RMF的概率化多模态融合模型
...
- 代理模态跨模态注意力
- KL散度 + 重建损失正则化
- 可选 emotion->valence 先验融合（sample-wise门控）
...
class AttentionRobustV13(nn.Module):
    """
    AttentionRobustV13 - 概率化多模态情感识别模型
    ...
    def __init__(self, args):
        super(AttentionRobustV13, self).__init__()
        ...
        # ==================== 情感-价度先验参数 (可选) ====================
        self.use_valence_prior = getattr(args, 'use_valence_prior', False)
        self.valence_consistency_weight = getattr(args, 'valence_consistency_weight', 0.05)
        self.valence_center_reg_weight = getattr(args, 'valence_center_reg_weight', 0.003)
        self.valence_prior_hidden_dim = getattr(
            args, 'valence_prior_hidden_dim', max(16, hidden_dim // 2)
        )
        self.valence_prior_gate_dropout = getattr(args, 'valence_prior_gate_dropout', 0.1)
        ...
        if output_dim1 == 6 and output_dim2 == 1:
            # MER2023 six-way emotion centers
        ...
        gate_in_dim = hidden_dim + output_dim1
        gate_hidden_dim = max(4, self.valence_prior_hidden_dim)
        self.valence_prior_gate_mlp = nn.Sequential(
            nn.Linear(gate_in_dim, gate_hidden_dim),
            nn.ReLU(),
            nn.Dropout(self.valence_prior_gate_dropout),
    ...
    def forward_vae(self, batch):
        ...
        vals_out, prior_val, _ = self.fuse_valence_with_emotion_prior(emos_out, raw_vals_out, features)
        ...
        if self.training:
            ...
            if prior_val is not None:
                ...
                interloss = interloss + self.valence_consistency_weight * consistency
                interloss = interloss + self.valence_center_reg_weight * center_reg
        else:
    ...
    def forward_original(self, batch):
        ...
        emos_out = self.fc_out_1(features)
        raw_vals_out = self.fc_out_2(features)
        vals_out, _, _ = self.fuse_valence_with_emotion_prior(emos_out, raw_vals_out, features)
