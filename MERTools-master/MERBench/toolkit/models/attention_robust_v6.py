'''
AttentionRobustV6 - 基于P-RMF V2的稳态迭代版本
...
- 代理模态跨模态注意力
- KL散度 + 重建损失正则化
- 修复模态Dropout对融合无效的问题（Dropout直接作用于mu）
- 支持动态KL warmup（训练前期更稳定）
...
class AttentionRobustV6(nn.Module):
    """
    AttentionRobustV6 - 概率化多模态情感识别模型 (V2稳态增强版)
    ...
    def __init__(self, args):
        super(AttentionRobustV6, self).__init__()
    ...
    def forward_vae(self, batch):
        ...
        # ========== 3. 不确定性加权融合 → 生成代理模态 ==========
        # 使用dropout后的均值和调整后的标准差计算权重，权重 ∝ 1/σ
